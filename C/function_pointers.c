...
pFunction functionArray[4] = {add,sub,mult};


// performs the given operation on elements of the arrays
int* performOp(int *a, int *b, int size, pFunction function);
...
int main( ){
  ...
  while(choice != 5)
  {
	...
	result = performOp(array1,array2,size,functionArray[j]);		
...
int* performOp(int *a, int *b, int size, pFunction function)
{
	...
	for(int i =0; i<size;i++)
		{
			*(resultArray+i)= function(a[i],b[i]);
		}
